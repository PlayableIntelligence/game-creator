...
Build a playable game prototype as fast as possible. This is `/make-game` without the doc rigor — just scaffold + implement the core loop. Get something on screen, then incrementally add layers with `/add-assets`, `/design-game`, `/add-audio`, `/game-deploy`, and `/monetize-game`.
...
>
> Or run `/make-game` next time for the full multi-session pipeline (idea → scaffold → development with milestones, ADRs, and per-feature iteration).
