...
- Read `docs/STATE.md` and `docs/gameplan.md` at the project root if they exist (written by the `/make-game` pipeline). Fall back to `progress.md` for older projects.
...
**In pipeline mode (called from another skill or `/make-game`):** auto-pick the top 2–3 and continue without prompting.
