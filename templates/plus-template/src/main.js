...
import { loadAsset, spawnAsset } from './assets/AssetLoader.js';
...
(async () => {
  try {
    ...
    // Step 8 demo — spawn a static asset (Robot statue) at an offset from
    // bbox centre to verify AssetLoader works.
    try {
      const robotAsset = await loadAsset('/assets/models/RobotExpressive.glb');
      const offX = ctr.x + 5;
      const offZ = ctr.z + 0;
      spawnAsset(robotAsset, scene, {
        position: [offX, world.fakeFloor.topY, offZ],
        rotation: [0, -Math.PI / 2, 0],
        scale: 1.0,
      });
    } catch (err) {
      console.warn('[main] demo asset spawn failed:', err);
    }

    // Step 12 — instantiate the user's Game class and fire lifecycle hooks.
    game = new Game();
